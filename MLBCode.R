...
df_schedules = df_schedules %>% 
  mutate(game_date_adj = if_else(stringr::str_sub(stringr::str_trim(date_game, "both"), -3, -1) %in% c("(1)", "(2)", "(3)"),
         ...
         date_format = strptime(format(strptime(game_date_adj, format = "%a, %b %d"), "%d/%m/%Y"), format = "%d/%m/%Y"),
    ...
    tabItem(tabName = "stats",
            fluidRow(
              box(
                ...
                selectInput("select_team", "Select Teams:", multiple = T, choices = unique(df_batting_stats$team_name)),
                width = 12, height = 130)
            ),
            #First Row of Plots:
            fluidRow(
...
server <- function(input, output) { 
  ...
  output$plot_g2 <- renderPlot({
    ...
    df2 = df_batting_stats %>% filter(team_name %in% input$select_team) 
    
    df2 %>% 
      ggplot(aes(x = team_name, y = batting_avg, label = paste0(round(batting_avg*100, 1), "%"))) + 
      geom_bar(stat = "identity", col = 'black', fill = "red", alpha = 0.34) + 
      theme_bw() + 
      geom_text(vjust = -0.8) + 
      ylim(c(0, 1.2)) + 
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, color = "black")) + 
      labs(x = "Team", y = "% Batting Average", title = "% Batting Avg by Team") + 
